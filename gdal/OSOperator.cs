...
namespace gdal
{
    ...
    class OSOperator
    {
        ...
        {
            var path = @"C:\TEMP\统一时点\三种变更\20200427三种变更.gdb";
            var driver = Ogr.GetDriverByName("FileGDB");

            //if (driver != null)
            //{
            //    var dsz = driver.Open(path, 1);
            ...
            try
            {
                pDataSource = driver.Open(path,1);
            }
            catch (Exception ex)
            {
            }
            ...
            for (int i = 0; i < pDataSource.GetLayerCount(); i++)
            {
                ...
                pLayer = pDataSource.GetLayerByIndex(i);
                featureDefn = pLayer.GetLayerDefn();
                for(int fldIdex=0; fldIdex < featureDefn.GetFieldCount(); fldIdex++)
                {
                   var fldDefn=featureDefn.GetFieldDefn(fldIdex);
                    if (fldDefn.GetName().ToLower() == "")
                    {
                        FieldDefn fieldDefn = new FieldDefn(fldDefn.GetName(), fldDefn.GetFieldType());

                        fieldDefn.SetWidth(15);
                        fieldDefn.SetPrecision(2);
                        //pLayer.re((fldIdex,fieldDefn,)
                    }
